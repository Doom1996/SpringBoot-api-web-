...
$(document).ready(function() {//se ejecuta todo el codigo una vez que se carga la pagina(
  ...
  $('#usuarios').DataTable();//Convierte la tabla HTML en una DataTable (paginación"paginas a tablas", búsqueda, orden)

    updateEmailUser();//muestra el email en el perfil de usuario

});

function updateEmailUser(){
    document.getElementById('txt-email-user').outerHTML = localStorage.email;//Actualiza el nombre del usuario en el perfil de la pagina
}
...
function getHeaders(){
    ...
    };//informacion del token,tomamos el token del localStorage y la enviamos en el header
}
