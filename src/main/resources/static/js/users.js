...
$(document).ready(function() {//se ejecuta todo el codigo una vez que se carga la pagina(
  ...
  $('#usuarios').DataTable();//aqui nombre de la tabla del html,DataTable(),hace que tenga algunas funcionalidades
});
...
let listaHtml = "";
for(let user of usersJson){//aqui se esta accediendo a un JSON se accede a propiedades.se llama cuando ocurre una accion del usuario
    let buttonDelete = `
        <a href="#"
  ...
  document.querySelector('#usuarios tbody').outerHTML = listaHtml;//codigo probado en consola de Firefox:Busca en la página un elemento HTML que cumpla exactamente esto.Modifica el contenido del elemento(solo para mi)
}
